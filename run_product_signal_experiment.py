"""OPTION8 — per-product routing signal (shortcut routing + hop-deficit feature).
...
from evaluation.live_inference import (
    ...
    compute_provenance_scores, compute_hop_deficit_factor, split_timeline,
)
...
def build_transactions(config: Dict[str, Any], seed: int, clustered: bool) -> pd.DataFrame:
    ...
    return inject_anomalies(
        ...
        cluster_counterfeit_by_manufacturer=clustered, shortcut_routing=True,
...
def run_seed(config: Dict[str, Any], models: Dict[str, Any], seed: int) -> Dict[str, Any]:
    ...
    base_s1 = compute_provenance_scores(evaluation, trust)
    factor = compute_hop_deficit_factor(burn_in, evaluation)
    s1 = {"s1_only": base_s1, "s1_x_deficit": base_s1 * factor}
    ...
    np.savez_compressed(
        cache_dir / f"seed{seed}_opt8.npz",
        cnn_cf=cached["counterfeit"]["cnn"], iso_cf=cached["counterfeit"]["iso"],
        cnn_clean=cached["clean"]["cnn"], iso_clean=cached["clean"]["iso"],
        s1_base_cf=s1["s1_only"][counterfeit], s1_base_clean=s1["s1_only"][clean],
        s1_def_cf=s1["s1_x_deficit"][counterfeit], s1_def_clean=s1["s1_x_deficit"][clean],
...
def main() -> None:
    ...
    started = time.time()
    print("#" * 78)
    print("OPTION8 — per-product routing signal (shortcut routing + hop-deficit feature)")
    ...
    for seed in seeds:
        ...
        per_seed.append(r)
        p, g = r["s1_only"], r["s1_x_deficit"]
        print(f"  seed {seed}: s1_only TPR={p['tpr']:.4f} FPR={p['fpr']:.4f} AUC={p['auc_pts']:.4f}"
              f" | s1_x_deficit TPR={g['tpr']:.4f} FPR={g['fpr']:.4f} AUC={g['auc_pts']:.4f}"
              f" | matchedFPR {p['matched_fpr']:.4f}->{g['matched_fpr']:.4f}", flush=True)
    ...
    d_fpr = (summary["s1_x_deficit"]["fpr"] - summary["s1_only"]["fpr"]) * 100
    d_tpr = (summary["s1_x_deficit"]["tpr"] - summary["s1_only"]["tpr"]) * 100
    d_auc = summary["s1_x_deficit"]["auc"] - summary["s1_only"]["auc"]
    print("-" * 78)
    print(f"{'delta (deficit-s1only)':<20}{d_tpr:>+9.2f}pp{d_fpr:>+9.2f}pp{'':>10}{d_auc:>+10.4f}")
    print("=" * 78)

    out_path = REPO_ROOT / "results" / args.out
    out_path.write_text(json.dumps(
        {"generated_at": time.strftime("%Y-%m-%dT%H:%M:%S"),
         "protocol": "OPTION8 shortcut routing + hop-deficit feature (DATA-GENERATION CHANGE)",
         "arm": "ARM4 clustered + shortcut_routing=True, published CNN + IF",
